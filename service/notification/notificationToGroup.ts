...
export async function notificationToGroupFromPatient(userToken: string,userName:string, message:number) {
  ...
  let {title,description}=notificationMap[message];
    ...
    const response = await fetch(`${BACKEND_URL}/api/notification/sendtogroupfrompatients`, {
      ...
      body: JSON.stringify({ userToken, title, body:description }),
    });
